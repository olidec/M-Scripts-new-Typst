...
// ── Palette ──────────────────────────────────────────────────
//
//  Two hue families, not seven. The old palette gave nearly every
//  environment its own hue and then handed the SAME teal to the
//  chapter title, the section rules, the theorem, the keybox and
//  the exercise — so the exercise had nothing left to distinguish
//  itself with. The grouping below is semantic instead:
//
//    THEORY  (forest green) — definition, theorem, keybox are
//        three weights of ONE hue, so they read as one category
//        with an internal ranking rather than as three unrelated
//        objects. `accent` is the middle weight and doubles as
//        document chrome (titles, section rules, table headers).
//
//    ACTIVITY (periwinkle) — exercise, exploration, ai-box: the
//        things a student DOES. The exercise separates from its
//        two siblings by GEOMETRY rather than hue — see the box
//        construction in exercise() below. Deliberately calm:
//        an alerting color on every exercise teaches anxiety.
//
//    RED — warning() and nothing else. Red appearing exactly once
//        per document is what makes it work as an alert at all.
//        In particular the NO CALC badge no longer borrows it.
//
//    GRAY — example, remark, quotebox, look-ahead: the asides.
//
//  Old names are all retained, so no chapter file needs editing.
#let accent = rgb("#38805f")   // mid forest — chrome + theorems
#let accent-bg = rgb("#e9f0ec")   // pale green fill
#let def-col = rgb("#1b4332")   // deep forest (definitions)
#let def-bg = rgb("#e9f0ec")   // same fill as theorems: one family
#let key-col = rgb("#0b2b1d")   // near-black green (key ideas)
#let key-bg = rgb("#d7e5dc")   // deeper fill — the loudest theory box
#let task-col = rgb("#4c5fa8")   // periwinkle (exercises)
#let task-bg = rgb("#eef0fa")   // pale periwinkle fill
#let expl-col = rgb("#5768ab")   // lighter periwinkle (explorations)
...
#let goal-bg = rgb("#eef4f0")   // pale green fill
#let ahead-col = rgb("#5f6b64")   // gray-green (look-aheads)
#let ahead-bg = rgb("#f0f2f1")   // pale gray fill
#let warn-col = rgb("#b3261e")   // red — warnings ONLY
#let warn-bg = rgb("#fbeceb")   // pale red fill
#let badge-col = rgb("#55605a")   // neutral ink for CALC / NO CALC
...
#let keybox(title: none, body) = context {
  if _hide-aux() { return }
  _bar-box(bar-color: key-col, fill-color: key-bg, [
    #if title != none [#text(weight: "bold", fill: key-col)[#title] \ ]
    #body
    #v(.1cm)
  ])
}
...
//
//  keep-together: true (default) → the whole exercise box, in both
//         chapter/main mode and solutions-booklet mode, is kept on
//         one page. This is the default because a split exercise
//         reads as two half-exercises: the question is the unit a
//         student works with, and handing them the first two parts
//         at the foot of one page and the rest overleaf makes them
//         page-flip while thinking.
//
//         The cost is white space — an exercise that doesn't fit in
//         the remaining space moves whole to the next page and
//         leaves a gap. That's usually the better trade, but not
//         always.
//
//         false → the box may split across a page break. Use it for
//         any exercise approaching a full page, and for long
//         solutions in the booklet. IMPORTANT: an unbreakable block
//         taller than the text area does not shrink to fit — it
//         overflows off the bottom of the page and the overflow is
//         lost. If an exercise or its solution is that long, either
//         pass keep-together: false or split it into two exercises.
//         Nothing warns you about this; check the built PDF.
//
//         Even with keep-together: false the header can never be
//         orphaned — it is a sticky, unbreakable sub-block, so a
//         break can only fall inside the body. See ex-header below.
// ────────────────────────────────────────────────────────────
#let exercise(
  chapter: "Unknown",
  level: "all",
  ...
  context {
    ...
    let dot(filled) = box(baseline: 15%, circle(
      radius: 2.3pt,
      fill: if filled { task-col } else { white },
      stroke: 0.6pt + task-col,
    ))
    ...
    // asking for something -- so it carries the stronger treatment.
    // "CALC" is merely permission and stays quiet.
    //
    // Both are now drawn in neutral ink rather than in warn-col. A
    // red badge sitting inside every hand-computation exercise made
    // red mean two different things (danger / do this by hand) and
    // put an alerting color on the exercises students are most
    // likely to find hard. The loaded state is still the louder of
    // the two, but it earns that through a solid fill and reversed
    // text rather than through a hue borrowed from the warnings.
    let calc-badge = if calculator == none { none } else {
      let (tag, fg, bg) = if calculator == true {
        ("CALC", badge-col, none)
      } else {
        ("NO CALC", white, badge-col)
      }
      box(
        baseline: 15%,
        inset: (x: 4pt, y: 1.5pt),
    ...
    // ── HEADER, as one indivisible unit ──────────────────────
    //
    //  breakable: false keeps the number, the badges and the effort
    //  note together; sticky: true refuses to let that unit be the
    //  last thing on a page. Together they remove both of the ways
    //  an exercise used to fall apart across a page boundary: a
    //  lone "Exercise 12" stranded at the foot of one page, and an
    //  effort note stranded at the head of the next.
    //
    //  The effort note has MOVED from the bottom of the box to the
    //  top. That is not only a typesetting fix: "expect to spend 20
    //  minutes, and being stuck is part of it" is calibration a
    //  student needs BEFORE starting, not consolation offered after
    //  they have already read the question and felt slow. To put it
    //  back at the bottom, drop `#eff` from this block and restore
    //  `#if effort-note != none [#v(0.4em) #effort-note]` after
    //  `#body` in the chapter-mode branch below.
    let ex-header(col, eff) = block(
      width: 100%,
      breakable: false,
      sticky: true,
    )[
      #grid(
        columns: (1fr, auto),
        text(weight: "bold", fill: col)[Exercise #n],
        align(right + horizon)[#tags],
      )
      #if eff != none [#v(0.25em) #eff #v(0.1em)]
    ]
    ...
    if _ex-mode.get() {
      ...
      grid(
        columns: (1fr, 1fr),
        align(left)[#text(weight: "bold", fill: task-col)[#chapter]],
        align(right)[
          #if tags != none [#tags #h(0.6em)]
          #text(fill: luma(100))[Exercise #n · #lvl-name]
        ],
      )
      // 2.5pt echoes the top rule of the exercise box in the lecture
      // notes, so the printed sheet and the notes are visibly the
      ...
      block(
        ...
        stroke: (
          top: 2.5pt + task-col,
          left: 0.6pt + task-col,
          right: 0.6pt + task-col,
          bottom: 0.6pt + task-col,
        ),
      )[
      ...
      // ── CHAPTER / MAIN MODE ──────────────────────────────────
      v(0.6em)
      // GEOMETRY, not just hue. A full hairline border with a heavy
      // top rule and SQUARE corners is what actually separates an
      // exercise from the theory boxes: every other environment in
      // this preamble is a 4pt left bar on a pale fill with rounded
      // corners, so two boxes of that shape read as the same kind of
      // object however far apart their hues are. The square corners
      // also survive grayscale printing and photocopying, which the
      // fill does not.
      block(
        width: 100%,
        breakable: not keep-together,
...
#let print-hints() = context {
  ...
  for entry in entries {
    grid(
      columns: (2.5cm, 1fr),
      gutter: 0.4em,
      text(weight: "bold", fill: task-col)[Ex. #entry.number],
      {
